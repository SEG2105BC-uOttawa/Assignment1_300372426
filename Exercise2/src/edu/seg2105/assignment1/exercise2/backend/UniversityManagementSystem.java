...
public class UniversityManagementSystem {
	...
	public void processCreateEmployee(String entity, String firstName, String lastName, double salary) {
		...
		if (entity.startsWith("PROFESSOR"))
			...
			employee = null; // The employee is not created
		}
	...
	public void processListEmployees(String entity) {
		...
		for (Employee employee : employees) {
			if (entity.startsWith("EMPLOYEE") || // print all employees if the entity is employee
					entity.startsWith("PROFESSOR") && employee instanceof Professor || // print only professors if the
																						// entity is professor
					entity.startsWith("ADMIN") && employee instanceof Administrator || // print only administrators if
																						// the
																						// entity is administor
					entity.startsWith("TA") && employee instanceof TeachingAssistant // print only administrators if the
																						// entity is teaching assistant
			)
	...
	public void processAssignInstructor(String id, String courseCode) {
		...
		if (success) { // check if I can assign the course to the instructor
			if (instructor instanceof Professor) {
				...
				// The instructor is a teaching assistant, therefore add them as a TA
	...
	public void processAssignAdministrator(String id, String task) {
		...
		if (administrator == null) {
			ui.display("Incorrect administrator id");
			return;
		}
		...
		ui.display("Task added to the administrator " + id);
		ui.display(administrator);
	}
